...
function NotificationContent({ t, message, type, linkType, linkDest }: NotificationProps) {
  return (
    <div
      ...
      } max-w-md w-full bg-deep-indigo shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5`}
    >
      <div className="flex-1 w-0 p-4">
        <div className="flex items-start">
          <div className="flex-shrink-0 pt-0.5">{getIcon(type)}</div>
          <div className="ml-3 flex-1">
            <p className="text-sm font-montserrat text-soft-silver">{message}</p>
            {linkType && linkDest && (
              <a
                href={getExplorerLink(linkType, linkDest).link}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-1 text-sm text-electric-blue hover:text-luminous-turquoise"
              >
                View {linkType}
              </a>
            )}
      ...
      <div className="flex border-l border-hover-grey">
        <button
          onClick={() => toast.dismiss(t.id)}
          className="w-full border border-transparent rounded-none rounded-r-lg p-4 flex items-center justify-center text-sm font-montserrat text-soft-silver hover:text-electric-blue focus:outline-none focus:ring-2 focus:ring-electric-blue"
        >
          Close
        </button>
      </div>
