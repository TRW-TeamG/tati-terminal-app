...
export function createWeb3JsEddsa(): EddsaInterface {
  ...
  const createKeypairFromSeed = (seed: Uint8Array): Keypair => fromWeb3JsKeypair(Web3JsKeypair.fromSeed(seed))

  const createKeypairFromFile = (path: string): Keypair => fromWeb3JsKeypair(Web3JsKeypair.generate())

  const createKeypairFromSolanaConfig = (): Keypair => fromWeb3JsKeypair(Web3JsKeypair.generate())

  const isOnCurve = (input: PublicKeyInput): boolean => Web3JsPublicKey.isOnCurve(toWeb3JsPublicKey(publicKey(input)))

  const findPda = (programId: PublicKeyInput, seeds: Uint8Array[]): Pda => {
    const [key, bump] = Web3JsPublicKey.findProgramAddressSync(seeds, toWeb3JsPublicKey(publicKey(programId)))
  ...
  const verify = (message: Uint8Array, signature: Uint8Array, publicKey: PublicKey): boolean =>
